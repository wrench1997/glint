...
func CheckSenseJsonp(args layers.PluginParam) (bool, *Jsonpinfo, error) {
	queryMap, _, err := UrlParser(args.Url)
	if err != nil {
	...
	if isCallback {
		//	referer： host 请求
		normalRespContent, _, err := GetJsResponse(args)
		if err != nil {
			return false, nil, err
		}
		...
		if isJsonpNormal {
			...
			noRefererContent, info, err := GetJsResponse(args)
			if err != nil {
				return false, nil, err
			}
...
func GetJsResponse(Param layers.PluginParam) (string, *Jsonpinfo, error) {
	...
	req1, resp1, err := sess.Get(Param.Url, Param.Headers)
	if err != nil {
		return "", nil, nil
	}
...
func JsonpValid(args *plugin.GroupData) (*util.ScanResult, bool, error) {
	util.Setup()
	...
	isvul, info, err := CheckSenseJsonp(Param)
	if err != nil {
	...
	if isvul {
		Result := util.VulnerableTcpOrUdpResult(Param.Url,
			...
			"middle",
			Param.Hostid, string(plugin.Jsonp))
		gd.Alert(Result)
