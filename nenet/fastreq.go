...
type ReqOptions struct {
	...
	RetryTimes    int64  // 重定向次数 0为默认值，-1 代表关闭不retry
...
func (sess *Session) doRequest(verb string, url string, headers map[string]string, body []byte) (*fasthttp.Request, *fasthttp.Response, error) {
	...
	// 设置重试次数
	retry := sess.ReqOptions.RetryTimes
	if retry == 0 {
		retry = defaultRetry
	} else if retry == -1 {
		retry = 0
	...
	fixBody, err := util.UnzipResponseBody(resp)
	if err != nil {
		if req != nil {
			req.ResetBody()
			req.Reset()
		...
		errmsg := fmt.Sprintf("util/requests.go:DoFasthttpRequest fasthttp client dealResponseBody error %s", string(req.RequestURI()))
		return nil, nil, errors.Wrap(err, errmsg)
	}

	resp.SetBodyRaw(fixBody)
