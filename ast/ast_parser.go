	...
	for {
		...
		case html.TextToken:
			...
			if field, ok := Tree.Max(); ok {
				field.Value.Content = util.Byte2Str(z.Text())
			}
...
func SearchInputInResponse(input string, body string) []Occurence {
	...
	Index := 0
	if len(body) == 0 {
		logger.Warning("SearchInputInResponse 获取body失败")
		return Occurences
	}
